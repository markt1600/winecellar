...
import {queuePoster} from '../lib/video-preview.mjs';
export default function ClipPlayer({clip}){
 ...
 useEffect(()=>{
  ...
  const start=()=>{if(started)return;started=true;queuePoster(source,clip,controller.signal).then(image=>{if(!controller.signal.aborted){setPoster(image);setStatus('ready');}},()=>{if(!controller.signal.aborted)setStatus('failed');});};
  let observer;
