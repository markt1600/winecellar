...
export async function GET(){
 ...
 try{const [d,c]=await Promise.all([readState(devicePath),readState(commandPath)]);return Response.json({device:d.value,command:c.value,ready:online(d.value),shutdownReady:online(d.value)&&d.value.shutdownReady===true},{headers});}
...
export async function POST(request){
 ...
 let b;try{const s=await request.text();if(s.length>256)throw Error();b=JSON.parse(s);if(!uuid(b.id)||![undefined,"reboot","shutdown"].includes(b.action))throw Error();}catch{return Response.json({error:'Invalid request'},{status:400,headers});}
 try{
  const c=await readState(commandPath);
  if(c.value?.id===b.id)return Response.json({ok:true,command:c.value},{headers});
  if(pending(c.value))return Response.json({error:'A power command is already in progress.'},{status:409,headers});
