...
export async function GET(){
 ...
 try{const [d,c]=await Promise.all([readState(devicePath),readState(commandPath)]);return Response.json({device:d.value,command:c.value,ready:online(d.value)},{headers});}
...
export async function POST(request){
 ...
 let b;try{const s=await request.text();if(s.length>256)throw Error();b=JSON.parse(s);if(!uuid(b.id))throw Error();}catch{return Response.json({error:'Invalid request'},{status:400,headers});}
 try{
  const c=await readState(commandPath);
  if(c.value?.id===b.id)return Response.json({ok:true,command:c.value},{headers});
  if(pending(c.value))return Response.json({error:'A reboot is already in progress.'},{status:409,headers});
  ...
  const now=Date.now();const command={id:b.id,status:'queued',createdAt:new Date(now).toISOString(),expiresAt:new Date(now+120000).toISOString(),bootId:d.value.bootId};
