...
function Chart({points,keys,start,end,range}){
  ...
  const step=points.length>1?Math.max(1,Date.parse(points[1].at)-Date.parse(points[0].at)):10000;
...
export default function Dashboard(){
 const [data,setData]=useState(null),[error,setError]=useState(''),[period,setPeriod]=useState('24h');
 ...
 if(!data||data.empty)return <section className="gate"><h2>{error?'Connection unavailable':data?.empty?'Waiting for the first upload':'Loading your cellar…'}</h2><p>{error||'Readings appear here after the Pi uploads them.'}</p></section>;
 ...
 <div className="legend"><span style={{color:fields[0][3]}}>● Bottle</span><span style={{color:fields[1][3]}}>● Ambient</span></div><Chart points={w.points} keys={['bottle_c','ambient_c']} start={w.start} end={w.end} range={validTargets?[targets.low,targets.high]:null}/>
 ...
 <h2>Humidity</h2><Chart points={w.points} keys={['humidity_pct']} start={w.start} end={w.end} range={validTargets?[targets.humidityLow,targets.humidityHigh]:null}/>
