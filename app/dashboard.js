...
export default function Dashboard({initialData=null,refresh=true}){
 ...
 return <><div className="journal"><span className={stale?'warning':''}>{stale?'● LOGGER DATA STALE':fields.some(([k])=>!Number.isFinite(latest[k]))?'● LOGGER ONLINE · SENSOR ISSUE':'● RECENT READINGS'} · {time(latest.observed_at)}</span><span>Cloud sync · {time(data.generatedAt)}</span></div><WifiStatus wifi={data.wifi} now={now}/>{error&&<p className="error">{error}</p>}
