...
export function retentionPlan(events,videos){
...
export async function pruneClips(api={list,del}){
 const [events,videos]=await Promise.all([allBlobs('cellar/v1/events/',api),allBlobs('cellar/v1/clips/',api)]);
 const plan=retentionPlan(events,videos);
